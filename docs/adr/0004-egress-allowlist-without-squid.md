...
- `images/contestant/firewall.nix` gets an `inet filter` `output` chain (`policy accept` — this is scoped to the `contestant` user, not the whole machine; `icpcadmin`/root keep unrestricted egress, since organizers use these same public IPs as SSH jump hosts to reach and manage sibling contestant/console machines for ansible and troubleshooting).
